...
LOGGER = set_logging(__name__)  # define globally (used in split_train_tph.py, val.py, detect.py, etc.)
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300, nms_type='default'):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if nms_type == 'DIoU':
            i = NMS(boxes, scores, iou_thres, DIoU=True)  # DIoU NMS
        elif nms_type == 'soft':
            i = my_soft_nms(boxes, scores, iou_thresh=0.5, sigma=0.5, score_threshold=0.25)  # soft NMS
        else:
            i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS

        if i.shape[0] > max_det:  # limit detections
            i = i[:max_det]
        if nms_type == 'merge' and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
