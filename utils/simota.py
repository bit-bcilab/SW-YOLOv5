...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Union Area
        w1, h1 = pred[:, 2], pred[:, 3] + eps
        w2, h2 = target[:, 2], target[:, 3] + eps
        area_u = area_p + area_g - area_i + eps

        iou = (area_i) / (area_u)

        if self.loss_type in ['GIoU', 'DIoU', 'CIoU', 'EIoU', 'SIoU']:
            c_tl = torch.min((pred[:, :2] - pred[:, 2:] / 2), (target[:, :2] - target[:, 2:] / 2))
            c_br = torch.max((pred[:, :2] + pred[:, 2:] / 2), (target[:, :2] + target[:, 2:] / 2))
            cw = (torch.max((pred[:, 0] + pred[:, 2] / 2), (target[:, 0] + target[:, 2] / 2))) - \
                 (torch.min((pred[:, 0] - pred[:, 2] / 2), (target[:, 0] - target[:, 2] / 2)))
            ch = (torch.max((pred[:, 1] + pred[:, 3] / 2), (target[:, 1] + target[:, 3] / 2))) - \
                 (torch.min((pred[:, 1] - pred[:, 3] / 2), (target[:, 1] - target[:, 3] / 2)))

            if self.loss_type == 'SIoU':  # SIoU Loss https://arxiv.org/pdf/2205.12740.pdf
                ...
                siou = iou - 0.5 * (distance_cost + shape_cost)  # SIoU
                loss = 1 - siou  # SIoU loss

            if self.loss_type in ['DIoU', 'CIoU', 'EIoU']:
                c2 = cw ** 2 + ch ** 2 + eps  # convex diagonal squared
                rho2 = ((target[:, 0] - pred[:, 0]) ** 2 +
                        (target[:, 1] - pred[:, 1]) ** 2)  # center distance squared

                # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
                if self.loss_type == 'DIoU':
                    diou = iou - rho2 / c2  # DIoU
                    loss = 1 - diou.clamp(min=-1.0, max=1.0)  # DIoU loss
...
class simOTA(nn.Module):
    def __init__(
        ...
        self.n_anchors = anchors.shape[1]  # 每层anchors的数量，anchor_free是1
    ...
    def forward(self, outputs_p, labels, imgs):
        ...
        for k in range(len(outputs_p)):
            ...
            if self.training:
                ...
                output, grid = self.get_output_and_grid(output, k, stride_this_level, torch.cuda.FloatTensor)
                x_shifts.append(grid[:, :, 0])
                y_shifts.append(grid[:, :, 1])
                expanded_strides.append(
                    torch.zeros(1, grid.shape[1]).fill_(stride_this_level).type(torch.cuda.FloatTensor))
    ...
    def get_losses(
        ...
        labels_tmp = torch.zeros(bbox_preds.shape[0], MAX_TEMP, 5)
